        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="grid gap-4">
            {problems.map((problem) => (
              <div key={problem} className="flex items-start gap-3 rounded-3xl bg-brand-soft p-4">
                <AlertCircle className="mt-0.5 size-5 shrink-0 text-brand-dark" />
                <span className="font-semibold text-brand-ink">{problem}</span>
              </div>
            ))}
            <Card className="mt-2 flex items-start gap-4 bg-gradient-to-br from-brand to-brand-dark text-white">
